...
const Candle = memo(() => {
  ...
  function setData(data: Array<any>) {
    ...
    setMaPrice({
      ma8Price: ma8Data.at(-1)?.value ?? 0,
      ma288Price: ma288Data.at(-1)?.value ?? 0
    })
  ...
  function getBackgroundColor(): string {
    const color = snap.isLight ? `255,255,255` : `31,38,47`
      ...
      {(isShowLoading) ?
        <div className={cx(style.loadingC, flexStyle.fcc)}>
          ...
          <div className={cx(style.legend)}>
            ...
            <span style={{ color: ma8Color }}>{'MA(8): ' + Number(maPrice.ma8Price).toLocaleString('en-US')}</span>
            <span style={{ color: ma288Color }}>{'MA(288): ' + Number(maPrice.ma288Price).toLocaleString('en-US')}</span>
          </div>
        </>)
      }
    </div>
